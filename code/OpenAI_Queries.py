...
def check_deployment():
    ...
    try:
        ...
        st.error(f"""LLM is not working.  
            Please check you have a deployment named {llm_helper.deployment_name} in your Azure OpenAI resource {llm_helper.api_base}.  
...
try:
    ...
    if 'custom_temperature' not in st.session_state:
        st.session_state['custom_temperature'] = float(os.getenv("OPENAI_TEMPERATURE", 0.75))
    ...
    with col1:
        st.image(os.path.join('images','shiseido.png'))
